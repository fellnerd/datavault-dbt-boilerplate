...
SELECT COUNT(*) FROM stg.ewb_sp_budget;              -- Fehler erwartet
...
```sql
SELECT (SELECT COUNT(*) FROM stg.ewb_sp_budget WHERE Konto IS NOT NULL AND Kostenstelle IS NOT NULL) AS quelle,
